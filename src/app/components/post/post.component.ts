...
export class PostComponent implements OnInit, OnDestroy {
  ...
  social: SocialMedium[] = [
    {
      name: "facebook",
      shortcut: "fb",
      urlBase: "https://facebook.com/sharer/sharer.php?u=",
    },
  ...
  sponsor: SocialMedium[] = [
    {
      ...
      shortcut: "oc",
      urlBase:
        "https://opencollective.com/screensavergallery/donate?interval=oneTime&amount=20",
    },
  ];
  ...
  ngOnInit() {
    ...
    if (!this.post) {
      ...
      this.routeSubscription = this.route.url.subscribe({
        next: (url: UrlSegment[]) => {
          ...
          // get page first
          const getPageSub: Subscription = this.wpService
            .getPageBySlug(slug)
            .subscribe({
              next: (pages: Page[]) => {
                ...
                if (pageBody) {
                  ...
                  const getPostSub: Subscription = this.wpService
                    .getPostBySlug(slug)
                    .subscribe({
                      next: (posts: Post[]) => {
    ...
    if (
      ...
      this.post.categories.includes(324)
    ) {
  ...
  private _cleanPostElm(): void {
    ...
    this.elm.nativeElement.classList.remove(
      ...this.elm.nativeElement.classList,
    );
    if (this.featuredImgContainer && this.featuredImgContainer.nativeElement)
      this.featuredImgContainer.nativeElement.style.backgroundImage = "none";
  }
  ...
  private _getFeautredImage(id: number): void {
    ...
    const mediaSubscribtion: Subscription = this.wpService
      .getMedia(id)
      .subscribe({
        next: (media: Media) => {
          ...
          if (media && media.source_url) {
            ...
            image.onload = () => {
              ...
              setTimeout(() => {
                this.featuredImgContainer.nativeElement.classList.add(
                  "fadeInFromNone",
                );
              }, 1000);
            };
